...
getAnnualTotalIncome() {    
    ...
    for(let i=0;i<=4;i++) {
        ...
        if( i === 0) {
            ...
            totalIncome = parseFloat(totalIncomeArray[i-1]) * ((parseFloat(this.markets[0].annual_rent_increase) * .01)+1);
        }
...
getAnnualTotalExpense() {
    ...
    for(let i=0;i<=4;i++) {
        ...
        if( i === 0) {
            ...
            totalExpense = parseFloat(totalExpenseArray[i-1]) * ((parseFloat(this.markets[0].annual_expense_increase) * .01)+1);
        }
...
getTotalVacancy() {
    ...
    for(let i=0;i<=4;i++) {
        vacancyAmt = totalIncome[i] * (this.markets[0].vacancy *.01);
...
getGOI() {
    ...
    for(let i=0;i<=4;i++) {
        ...
        grossOperatingIncomeArray.push(Math.round(grossOperatingIncome*100)/100);
    }
...
getAnnualCostBasis() {
    const costBasis = parseFloat(this.financials[0].offer_amount) + parseFloat(this.financials[0].closing_amount);
...
getMortgageInterest() {
    ...
    this.mortgages.forEach(value => {
        ...
        for(let i=0;i<=4;i++) {
            for(let x=0;x<12;x++) {
                intAmt = Math.round((principal * ((parseFloat(value.rate)*.01)/12))*100)/100;
...
getTaxableIncome() {
    ...
    for(let i=0;i<=4;i++) {
        ...
        taxIncomeArray.push(Math.round((noi[i] - depreciation[i] - intCalc)*100)/100);
        intCalc=0;
    }
...
getTaxesDue() {
    ...
    for(let i=0;i<=4;i++) {
        taxesDue = Math.round((taxableIncome[i] * (parseFloat(this.financials[0].tax_rate)*.01))*100)/100;
        if (taxesDue <= 0) {
            taxesDue = 0;
        }
...
getPropertyValue() {
    ...
    for(let i=0;i<=4;i++) {
        if(i === 0) {            
            propertyValue = ((this.markets[0].appreciation * .01) + 1) * this.financials[0].property_value;
        } else {
            propertyValue = ((this.markets[0].appreciation * .01) + 1) * propertyValueArray[i-1];
        }
...
getGrossMultiplier() {
    ...
    const grossMultiplier = Math.round((this.financials[0].offer_amount / (grossRent[0]))*100)/100;
...
getIRR() {
    ...
    const irr = 100*(Math.round(((noi[0]-totalMortgagePayment[0])/totalMorgageAmount)*10000)/10000);
    const irrPct = irr + '%';
...
getCurrentLTV() {
    ...
    ltv = 100*(Math.round((totalPrin/this.financials[0].property_value)*10000)/10000);
...
getOwnershipPercentage() {
    ...
    for (let i=0;i<=4;i++) {
        ...
        ownPercent = 100*(Math.round(((propertyValue[i] - totalPrin)/propertyValue[i])*10000)/10000);
...
getCOCBeforeTax() {
    ...
    if (downPayment !== 0) {
        for (let i=0;i<=4;i++) {
            cashOnCashBeforeTax = (Math.round((beforeTaxCashflow[i]/downPayment)*10000)/10000)*100;
...
getCOCAfterTax() {
    ...
    if (downPayment !== 0) {
        for (let i=0;i<=4;i++) {
            cashOnCashAfterTax = (Math.round((afterTaxCashflow[i]/downPayment)*10000)/10000)*100;
...
getAppreciation() {
    ...
    for(let i=0;i<=4;i++) {
        if(i === 0) {            
            propertyAppreciation = ((this.markets[0].appreciation * .01)) * this.financials[0].property_value;
        } else {
            propertyAppreciation = ((this.markets[0].appreciation * .01)) * propertyValue[i-1];
        }
        propertyAppreciationArray.push(Math.round(propertyAppreciation*100)/100);
    }
...
getRORLoanReduction() {
    ...
    if (downPayment !== 0) {
        for(let i=0;i<=4;i++) {
            ...
            rorLoanReduction = 100*(Math.round(((totalPrin)/downPayment)*10000)/10000);
...
getRORAppreciation() {
    ...
    if (downPayment !== 0) {
        for(let i=0;i<=4;i++) {            
            rorAppreciation = 100*(Math.round(((appreciation[i])/downPayment)*10000)/10000);
...
getRORBeforeTaxCF() {
    ...
    if (downPayment !== 0) {
        for(let i=0;i<=4;i++) {            
            rorBeforeTaxCF = 100*(Math.round(((beforeTaxCashflow[i])/downPayment)*10000)/10000);
